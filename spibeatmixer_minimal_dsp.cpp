		...
		if(pInstrumentSet && pInstrumentSet->HasOneInstrument()) 
		{
			...
			//{
				...
				*/
				...
				float* audioData[2];
				audioData[0] = new float[pSilentWavSet->numSamples];
				audioData[1] = new float[pSilentWavSet->numSamples];
				...
				params[2] = 1.25; // Q
				f->setParams (params);
				f->process (pSilentWavSet->numSamples, audioData);
				
				if(audioData[0]) delete[] audioData[0]; 
				if(audioData[1]) delete[] audioData[1]; 
